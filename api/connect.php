...
if ($action === 'connect') {
    ...
    if ($mode === 'ASL' || $mode === 'ECHO') {
        ...
        if ($directUiMode === 'ECHO') {
            validate_echolink_target_or_fail($digitsOnlyTarget, static function (string $message, int $statusCode): never {
                $_SESSION['last_status'] = $message;
                respond(session_payload($_SESSION['last_status']), $statusCode);
            });
        }
