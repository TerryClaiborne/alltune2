...
function last_tracked_allstar_ui_mode(): string
{
    $node = last_tracked_allstar_node();
    return $node !== '' ? tracked_allstar_ui_mode($node) : 'ASL';
}

function sync_last_direct_target_from_tracking(): void
{
    $remainingTracked = last_tracked_allstar_node();

    if ($remainingTracked !== '') {
        $_SESSION['last_mode'] = last_tracked_allstar_ui_mode();
...
function session_payload(string $statusText, array $extra = []): array
{
    return array_merge([
        ...
        'bm_receive_active' => bm_receive_is_active(),
        'dvswitch_link_active' => !empty($_SESSION['dvswitch_autoloaded']) || !empty($_SESSION['dmr_ready']) || normalize_mode((string) ($_SESSION['last_mode'] ?? '')) === 'YSF' || bm_receive_is_active(),
    ], $extra);
}
...
if ($action === 'disconnect_selected') {
    ...
    if ($hasRealDvSwitchNode && $selectedNode === $dvSwitchNode) {
        ...
        sync_last_direct_target_from_tracking();
...
if ($action === 'connect') {
    ...
    if ($mode === 'TGIF') {
        ...
        if ($currentNetwork !== 'TGIF' || !$dmrReady) {
            if ($currentNetwork !== '' || normalize_mode((string) ($_SESSION['last_mode'] ?? '')) === 'YSF' || !empty($_SESSION['dvswitch_autoloaded'])) {
                disconnect_dvswitch_runtime($myNode, $dvSwitchNode);
                clear_runtime_targets();
            }
            ...
            dvswitch_tune($tgifPassword . '@tgif.network:62031');
            pause_seconds(10.0);
...
if ($trackedAllstarNode !== '') {
    ...
    $trackedUiMode = last_tracked_allstar_ui_mode();
