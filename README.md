...
Version 1.20.0 is a major update. The old two-step BrandMeister and TGIF workflow has been replaced with a much more usable one-step connect flow. BrandMeister receive handling now uses an AllTune2-owned backend helper together with a local AllTune2 stfu/STFU runtime copy, making the BM receive path part of the AllTune2 project structure instead of depending on the older separate STFU web-panel workflow. This release is intended for ASL3 / Debian / DVSwitch systems using the expected default layout and has been tested on the author’s working Raspberry Pi nodes. The integrated BM receive backend, local stfu/STFU binary, installer behavior, and sudoers setup all assume that type of environment, so systems with different architectures, paths, or custom layouts may require adjustment before installation or update.
