...
Enter or load the AllStar node.
Press Connect once.
...
Edit `/var/www/html/alltune2/config.ini` and set real values.
Confirm the sudoers file exists and is valid.
Open `/alltune2/public/` in the browser.
Test BM, TGIF, YSF, AllStar, DVSwitch auto-load, and disconnect actions.
Confirm favorites save correctly.
Confirm direct AllStar nodes show correctly in Live Status.
